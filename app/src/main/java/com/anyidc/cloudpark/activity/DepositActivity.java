...
public class DepositActivity extends BaseActivity {
    ...
    protected void initData() {
        initTitle("缴纳押金");
        depositNum = getIntent().getFloatExtra("depositNum", 0f);
    ...
    private void recharge() {
        switch (payType) {
            case 1:
                getTime(Api.getDefaultService().alPay("缴纳押金", "缴纳押金", String.valueOf(depositNum)
                    ...
                    public void onSuccess(BaseEntity<AlPayBean> baseEntity) {
                        Runnable payRunnable = () -> {
                            ...
                            result.put("num", String.valueOf(depositNum));
                            result.put("from", "0");
                ...
                break;
            case 2:
                getTime(Api.getDefaultService().wxPay("缴纳押金", "缴纳押金", String.valueOf(depositNum)
                        , 2, payType, null, null), new RxObserver<BaseEntity<WxPayBean>>(this, true) {
                    @Override
                    public void onSuccess(BaseEntity<WxPayBean> baseEntity) {
                        WXPayEntryActivity.setNum(String.valueOf(depositNum));
                        WXPayEntryActivity.setFrom(0);
